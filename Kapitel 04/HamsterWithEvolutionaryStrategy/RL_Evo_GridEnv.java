 ...
 */
public class RL_Evo_GridEnv extends Territory
{
    public static final double rewardForAGrain = 1; 
    public static final double rewardTrap = -1;
    public static final double rewardPerTransition = -0.01; //-0.1;
     ...
     */
    public RL_Evo_GridEnv(String[] fieldDescription)
    {
    ...
    {
        double reward = 0;
        List objects = this.getObjectsAt(x,y,Grain.class);
        if  ( objects.size()>0 ){
            for (Object grainpile : objects){ 
                reward+=((Grain)grainpile).getNumber()*RL_Evo_GridEnv.rewardForAGrain; 
            }
        }
        
        objects = this.getObjectsAt(x,y,Trap.class);
        if  ( objects.size()>0 ){
            reward+=(objects.size()*RL_Evo_GridEnv.rewardTrap);
        }
    ...
    /**
     * Returns a list of possible actions for a given field of the territory.
     * @param x column in the gridworld
     * @param y row in the gridworld
     */
     ...
     */
    public ArrayList <Integer> coursesOfAction(String s)
    {
        return coursesOfAction(Hamster.getSX(s),Hamster.getSY(s));
    }
    ...
    {
        List objects = (List)this.getObjectsAt(x,y,Wall.class);
        if (( objects.size()==0 ) &&
                ((x>=0) && (x<this.getWidth())) && 
